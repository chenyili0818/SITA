/-
This file is part of project Optlib (https://github.com/optsuite/optlib)
...
theorem deriv_function_comp_segment_aux'' {g : E → ℝ} {f : E → (E →L[ℝ] ℝ)}
  ...
  have hg : HasFDerivAt (fun z ↦ (f z) (y - x)) ((f' (x + t₀ • (y - x))) (y - x)) (x + t₀ • (y - x)) := by
    ...
    specialize hf1 pos
    suffices ∀ᶠ (x_1 : E) in nhds 0, ‖(f (x + t₀ • (y - x) + x_1)) (y - x) - (f (x + t₀ • (y - x))) (y - x)
...
lemma expansion_fderiv_second (s : Set E) (hs : Convex ℝ s)
  ...
  have g1 : ∀ t ∈ Set.Icc 0 1, HasDerivAt g (g' t) t := by
    exact fun t a ↦ deriv_function_comp_segment' s hs hf x y (interior_subset hx) (interior_subset hy) t a
  ...
  use t; constructor; exact ⟨ht1, ht2⟩
  simp [g, g', g''] at ht3
  simpa
...
lemma gradient_of_inner_const' (x : E) (a : E):
    HasGradientAt (fun x ↦ (inner x a : ℝ)) a x := by
  have : (fun x ↦ (inner x a : ℝ)) = fun x ↦ (inner a x : ℝ) := by
