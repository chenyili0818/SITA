...
theorem f_subdiff_closed (f : E → ℝ) (x : E) : IsClosed (f_subdifferential f x) := by
  ...
  have hh : f_subdifferential f x =
      {u | ∀ ε > 0, ∀ᶠ (y : E) in 𝓝 x, f y - f x - (inner u (y - x)) ≥ -ε * ‖y - x‖} := by
    ext u
  ...
  have tasubset': ta ⊆ {x_1 | f x_1 - f x - (inner v (x_1 - x)) ≥ -ε * ‖x_1 - x‖} := by
    intro y yinta; rw[mem_setOf, ge_iff_le]
    rw[← add_sub_cancel a v, inner_add_left, ← sub_sub, sub_eq_add_neg (f y - f x - inner a (y - x))]
...
theorem convex_f_f_subdiff_eq_subdiff (f : E → ℝ) (x : E)
  ...
  · intro h
    ...
    intro y
    have hvnin: ∀ (n : ℕ), f y ≥ f (u n) + inner (vn n) (y - (u n)):=by
      intro n
      specialize hvn n
    ...
    have tendsto: Tendsto (fun n ↦ f (u n) +
        inner (vn n) (y - (u n))) atTop (𝓝 (f x + inner v (y - x))):=by
