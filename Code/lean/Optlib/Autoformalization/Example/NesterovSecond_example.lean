import Optlib.Autoformalization.Template.NesterovSecond_template
...
def Lasso_problem.l (_ : Lasso_problem A b mu) : NNReal :=
  ‖(Matrix.toEuclideanLin ≪≫ₗ LinearMap.toContinuousLinearMap) (Aᵀ * A)‖₊

instance Lasso_problem.composite_problem (self : Lasso_problem A b mu) :
    composite_problem self.f self.g where

class Nesterov_second_fix_stepsize_Lasso (pro : Lasso_problem A b mu) (x₀ : EuclideanSpace ℝ (Fin n)) where
  hl : pro.l > (0 : ℝ)
  ...
  update2 : ∀ k : ℕ+,
    let grad : EuclideanSpace ℝ (Fin n) := Aᵀ *ᵥ (A *ᵥ z k - b)
    w k = y (k - 1) - (t k / γ k) • grad
  update3 : ∀ k : ℕ+, y k =
    fun i => Real.sign (w k i) * (max (abs (w k i) - (t k / γ k) * mu) 0)
  update4 : ∀ k : ℕ+, x k = (1 - γ k) • (x (k - 1)) + γ k • y k
...
lemma Nesterov_second_fix_stepsize_Lasso.update_cor (self : Nesterov_second_fix_stepsize_Lasso pro x₀) :
    ∀ (k : ℕ+), prox_prop ((self.t k / self.γ k) • pro.g)
    (self.y (k - 1) - (self.t k / self.γ k) • gradient pro.f (self.z k)) (self.y k) := by
  intro k
  unfold Lasso_problem.g
  apply norm_one_proximal
  · exact rfl
  · apply div_pos; rw [self.teq k]; simp [self.hl]; simp [self.γeq k]; linarith
  · exact pro.hmu
  · intro i
    rw [pro.gradient_f]
    rw [← self.update2 k]
...
instance Nesterov_second_fix_stepsize_Lasso.Nesterov_second_fix_stepsize (self : Nesterov_second_fix_stepsize_Lasso pro x₀) :
    Nesterov_second_fix_stepsize (Lasso_problem.composite_problem pro) x₀ where
  hl := self.hl
  x := self.x
  y := self.y
  z := self.z
...
theorem Lasso_convergence (alg : Nesterov_second_fix_stepsize_Lasso pro x₀)
  ...
  intro k
  apply Nesterov_second_fix_stepsize_converge (alg := alg.Nesterov_second_fix_stepsize)
